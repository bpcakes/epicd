...
`status --json` emits the current status schema. `agentSettings` contains effective settings and `agentPreferences` records inheritance choices. Session identities appear only in the role-keyed `agentSessions` object; the old thread-ID aliases are removed. When a controller owns a run, `controllerLease` reports its PID, opaque lease ID, acquisition time, and whether that exact process identity is still alive. Persisted run JSON must contain every required field and use the current session contract; old fields, schema version 1 and inferred historical defaults are unsupported.

Human-readable status includes the run and controller lease identities required by `epicd unlock`. If a persisted row is invalid, human status reports it while continuing to show other runs. Corrupt rows may be quarantined after inspection; this build refuses to quarantine rows declaring a newer state schema. JSON status omits invalid rows, writes a warning to stderr and returns a nonzero exit status. No command upgrades or silently rewrites an old run. Human status remains successful after reporting invalid rows so it can serve as an inspection command.
