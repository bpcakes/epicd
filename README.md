...
Both SDK and native Herdr browser harnesses now run an unauthenticated local-command prerequisite probe using their selected native Codex executable. They fail explicitly before creating a run, database, or Herdr test server when private scratch or local-server access is unavailable; they do not skip the acceptance or count this failure as success. Verify that guard with `EPICD_CODEX_CONFINEMENT=1 npm test -- test/browser-worker-preflight.integration.test.ts`. This is a permission-profile probe, not proof of native TUI delivery. The next prerequisite is kernel-mediated worker validation, followed by end-to-end diagnostic/recovery acceptance, not broader host-network permissions.
