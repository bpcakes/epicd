...
This branch has one orchestrator controller. There is no legacy phase dispatcher, compatibility mode, state conversion, or database migration. Current storage format is 31. Use a fresh state path; unsupported existing data is left intact.
...
The authenticated `EPICD_LIVE_DELIVERY_SCENARIO=browser` acceptance is not ready to run. Its first SDK attempt failed after 24 minutes: independent review could not obtain the original browser-command execution record. The implementer's command had stopped at the helper's hardcoded `/tmp` path, before authentication. The helper now honors private `TMPDIR`, but a real confined probe then reached Playwright's local server and failed with `listen EPERM`. Ordinary worker commands cannot open network listeners; the passing kernel-validation fixture uses a separate private network environment. The new transcript reader recovers the original missing tool result from that retained SDK run, and fresh authenticated SDK/native tests exercise failed-command capture. That diagnostic evidence does not retroactively approve the failed run or demonstrate browser recovery.

Both SDK and native Herdr browser harnesses currently run an unauthenticated local-command prerequisite probe using their selected native Codex executable. They fail explicitly before creating a run, database, or Herdr test server when private scratch or local-server access is unavailable; they do not skip the acceptance or count this failure as success. Verify that guard with `EPICD_CODEX_CONFINEMENT=1 npm test -- test/browser-worker-preflight.integration.test.ts`. This is a permission-profile probe, not proof of native TUI delivery. The kernel-mediated `run_diagnostic_check` capability is implemented: the mechanical browser contract now records a real authentication failure without the database binding, then uses separately authorized fixture creation and required validation to obtain a green browser result. The next step is adapting and exercising the authenticated worker handoff, not granting broader host-network permissions.

Once those prerequisites are implemented, the browser scenario still requires the same four variables and the existing assertions: a real completed-code/failed-browser report, diagnostic follow-up, configuration inspection, observed database absence, authorized creation, green kernel browser evidence before review, verified delivery, and preserved test/oracle/dependency files. Only the initial faulty claim shape is a deliberate instruction; no provider report or recovery strategy is injected. Existing SDK/native browser deadlines remain 30/60 minutes, and test-owned database data and run artifacts are retained. The failed attempt and remaining requirements are recorded in the plan.
