...
Every `run_review` request includes `references` (use `[]` when none are needed). Select up to eight bounded pages: `{ kind: "action", actionId, offset, limit }` for a settled action's redacted request/result, or `{ kind: "artifact", artifactId, offset, limit }` for an immutable retained diagnostic. Offsets count UTF-16 characters in the redacted view; limits are 1–4000. Pages together must fit 32 KiB, and the complete review context still has its 48-KiB bound. The kernel resolves references within this run, freezes their digest at admission, and supplies the primary records directly to the reviewer. Running/indeterminate actions, missing/foreign IDs and out-of-range pages are rejected. Gaps, truncation and diagnostic provenance remain explicit. These references cannot satisfy required checks or resolve findings: the current review context's validation/finding arrays still control those result fields.
...
The SDK attempt at `b1b1b08` exercised the kernel handoff and genuine authentication failure, authorized fixture creation, a reviewed private commit, and green browser checks at both candidate and actual commit revisions. It still failed the 30-minute deadline after 109 decisions: exact-revision review required primary historical records that were difficult to retrieve through clipped inspection previews. It paused with all 110 turns stopped; the test-owned PostgreSQL server is stopped and its data/run journal are retained. There was no completed delivery, and later acceptance assertions were not reached. `inspect_action` addresses the observed retrieval gap; its paging, redaction, cold recovery, cross-run isolation and changed-view regressions passed, as did bounded real Astra schema checks in SDK and native Herdr. The full browser acceptance has not been rerun with this fix. A separate diagnostic follow-up to an existing reviewer remains unavailable; the current kernel requires formal review turns for that agent.
