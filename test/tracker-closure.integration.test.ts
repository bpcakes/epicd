...
describe.skipIf(process.platform !== "linux")("verified task closure", () => {
  ...
  it("snapshots schema eleven without retrofitting claim provenance or rewriting old prompt and graph hashes", async () => {
    const s = await closureFixture();
    const run = s.authority.runId;
    const prepared = s.journal.agents.prepareTurn(
      s.authority,
      s.writer,
      randomUUID(),
    ...
    s.journal.agents.cancelPreparedTurn(s.authority, prepared.identity);
    const db = new Database(s.path);
    try {
      // Model an actual schema-11 payload, before either optional field existed.
      const assignment = s.journal.agents.assignment(run, s.writer.assignmentId);
      delete assignment.trackerClaim;
      db.prepare("UPDATE agent_assignments SET record_json = ? WHERE assignment_id = ?").run(
        JSON.stringify(assignment),
        assignment.assignmentId,
      );
      const turn = s.journal.agents.turn(run, prepared.identity);
      delete turn.prompt.assignment.trackerClaim;
      ...
      for (const row of db.prepare("SELECT record_json FROM tracker_snapshots").all() as {
        record_json: string;
      }[]) {
        const snapshot = JSON.parse(row.record_json);
        for (const issue of snapshot.graph.issues) {
          delete issue.closedAt;
          delete issue.closeReason;
          delete issue.closedBySession;
          delete issue.updatedAt;
        }
        snapshot.digest = digestJson(snapshot.graph);
        db.prepare("UPDATE tracker_snapshots SET record_json = ? WHERE snapshot_id = ?").run(
          JSON.stringify(snapshot),
          snapshot.snapshotId,
        );
      }
      db.exec("UPDATE orchestration_schema SET version = 11");
      const tables = [
        "tracker_operations",
        "tracker_snapshots",
        "agent_assignments",
      ...
      const upgraded = s.reopen();
      expect(db.prepare("SELECT MAX(version) AS version FROM orchestration_schema").get()).toEqual({
        version: 12,
      });
      tables.forEach((table, index) => {
        expect(db.prepare(`SELECT * FROM ${table} ORDER BY rowid`).all()).toEqual(original[index]);
      });
      expect(upgraded.orchestration.agents.turn(run, turn.identity)).toEqual(turn);
      expect(
        upgraded.orchestration.agents.assignment(run, assignment.assignmentId),
      ).not.toHaveProperty("trackerClaim");
      const graph = upgraded.orchestration.tracker.snapshot(run);
      expect(graph.digest).toBe(digestJson(graph.graph));
      expect(graph.graph.issues[0]).not.toHaveProperty("closedAt");
      expect(upgraded.orchestration.tracker.assertTaskOwned(run, s.taskId)).toBeDefined();
      const backupPath = readdirSync(s.root).find((name) => name.includes("before-orchestration"))!;
      const backup = new Database(join(s.root, backupPath), { readonly: true });
      try {
        expect(
          backup.prepare("SELECT MAX(version) AS version FROM orchestration_schema").get(),
        ).toEqual({ version: 11 });
        tables.forEach((table, index) => {
          expect(backup.prepare(`SELECT * FROM ${table} ORDER BY rowid`).all()).toEqual(
            original[index],
          );
        });
      } finally {
