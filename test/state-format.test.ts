...
describe("hard-cut state format", () => {
  ...
  it("creates the current schema once and reopens current runs without rewriting their state", () => {
    ...
    expect(db.prepare("SELECT version FROM orchestration_schema").all()).toEqual([{ version: 42 }]);
    expect(
  ...
  it.each([
    ...
    { label: "previous format without typed primary record references", versions: [34] },
    { label: "previous format without independent fixture-command stop receipts", versions: [35] },
    { label: "previous format without complete validation-worker proof", versions: [36] },
    { label: "previous format without recoverable workspace disposal", versions: [37] },
    { label: "previous format without full review evidence packets", versions: [38] },
    { label: "previous format without complete capture-worker proof", versions: [39] },
    { label: "previous format without grouped workspace creation proof", versions: [40] },
    { label: "previous format without complete publication worker proof", versions: [41] },
    { label: "newer format", versions: [43] },
