...
describe("hard-cut state format", () => {
  ...
  it("creates the current schema once and reopens current runs without rewriting their state", () => {
    ...
    expect(db.prepare("SELECT version FROM orchestration_schema").all()).toEqual([{ version: 22 }]);
    expect(
  ...
  it.each([
    ...
    { label: "older format", versions: [21] },
    { label: "newer format", versions: [23] },
    { label: "multiple format markers", versions: [21, 22] },
