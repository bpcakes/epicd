...
export function buildOrchestratorContext(kernel: ActionKernel, runId: string): OrchestratorContext {
  ...
  const context: OrchestratorContext = {
    ...
    constraints: [
      ...
      "A declared fixtureValidation bridge lets run_validation use the exact run-created host fixture only with a separate operator SQL-access grant and fresh restricted-role checks. Creation/inspection grants do not grant repository SQL access. If a fixture access remains unsettled, inspect_fixture_access/reconcile_fixture_access observe it without replaying commands; local process stop alone cannot prove a PostgreSQL backend stopped. Reconciliation never upgrades failed evidence.",
